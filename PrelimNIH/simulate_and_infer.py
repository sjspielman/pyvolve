...
############## GLOBAL ###############
molecules = Genetics()
treefile = path_to_sim + "trees/100.tre"
my_tree = readTree(file=treefile)
length = 100
codonParams = {'mu': {'AC': 1., 'AG': 1., 'AT': 1., 'CG': 1., 'CT': 1., 'GT': 1.}}
zero = 1e-8
...
outfile = "equal_bycodon_positive_omega.txt"
outf = open(outfile, 'w')
outf.write('num_aa\tsimulated_w\tderived_w\n')
...
for i in constrained_propensities_columns:
	...
	for w in omegas_positive:
		...
		# Evolve
		print "Evolving"
		seqfile = 'Simulated_52414/simseqs_' + numaa + '_' + str(w) + '.fasta'	#this turned out poorly because they all got overwritten. oh well. can always redo very easily.	
		...
		print "Deriving"
		derivedw = deriveAverageOmegaAlignment(seqfile, f)

		# And save it all
		outf.write(numaa+'\t'+str(w)+'\t'+str(derivedw)+'\n')
outf.close()
