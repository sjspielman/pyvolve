...
# Define an amino-acid model, as a Model object. The first argument should be either "JTT", "WAG", or "LG" (available empirical matrices in Pyvolve). Provide a parameters dictionary with key "state_freqs" to customize model. The provided frequency list must sum to 1! (Note that you can use the EmpiricalModelFrequencies class to use the default empirical model frequencies. See the user manual for details!)
...
my_model = Model("LG", {"state_freqs": aa_freqs)
