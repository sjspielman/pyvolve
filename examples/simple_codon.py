...
# Define a codon model, as a Model object. The first argument can be either:
...
# Codon models require you to specify a second argument to Model, a dictionary of parameters. You must specify dN/dS using either "omega" (for the full ratio), or "beta" for dN and "alpha" for dS, as shown below. Either dictionary would be acceptable.
parameters_omega = {"omega": 0.65}
parameters_alpha_beta = {"beta": 0.65, "alpha": 0.98} # Corresponds to dN/dS = 0.65 / 0.98
my_model = Model("MG", parameters_alpha_beta)

# Assign the model to a Partition. The size argument indicates to evolve 250 positions (for a codon alignment, this means 250 codons, i.e. 750 nucleotide sites)
