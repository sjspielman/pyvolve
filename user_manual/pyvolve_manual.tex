...
The callable \code{Evolver} class makes simulating replicates of given modeling scheme straight-forward: simply define an \code{Evolver} object, and then call this object inside of a for-loop as many times as needed.
...
\begin{figure}[h]
	\centerline{\includegraphics[width=3in]{yang_neutral_scaling.pdf}}
...
Therefore, Pyvolve allows you to specify some amount of noise in the branch lengths using the keyword argument \code{noisy\_branch\_lengths = True} when \emph{defining} (not calling!) an \code{Evolver} object:
\begin{lstlisting}[float=h]
